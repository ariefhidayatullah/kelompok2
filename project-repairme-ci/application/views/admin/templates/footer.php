   <!-- Main Footer -->
   <footer class="main-footer">
     <strong>RepairMe &copy; 2019 <a href="http://adminlte.io">Repairme</a>.</strong>
     Situs Untuk Mencari Tempat Perbaikan Terbaik.
     <div class="float-right d-none d-sm-inline-block">
       <b>Versi</b> 1.0.0
     </div>
   </footer>
   </div>
   <!-- ./wrapper -->

   <!-- REQUIRED SCRIPTS -->
   <!-- Bootstrap -->
   <script src="<?= base_url(); ?>assets/panel-master/plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
   <!-- overlayScrollbars -->
   <script src="<?= base_url(); ?>assets/panel-master/plugins/overlayScrollbars/js/jquery.overlayScrollbars.min.js"></script>
   <!-- Toastr -->
   <script src="<?= base_url(); ?>assets/panel-master/plugins/toastr/toastr.min.js"></script>
   <!-- AdminLTE App -->
   <script src="<?= base_url(); ?>assets/panel-master/dist/js/adminlte.js"></script>

   <!-- OPTIONAL SCRIPTS -->
   <script src="<?= base_url(); ?>assets/panel-master/dist/js/demo.js"></script>

   <!-- PAGE PLUGINS -->
   <!-- jQuery Mapael -->
   <script src="<?= base_url(); ?>assets/panel-master/plugins/jquery-mousewheel/jquery.mousewheel.js"></script>
   <script src="<?= base_url(); ?>assets/panel-master/plugins/raphael/raphael.min.js"></script>
   <script src="<?= base_url(); ?>assets/panel-master/plugins/jquery-mapael/jquery.mapael.min.js"></script>
   <script src="<?= base_url(); ?>assets/panel-master/plugins/jquery-mapael/maps/usa_states.min.js"></script>
   <!-- ChartJS -->
   <script src="<?= base_url(); ?>assets/panel-master/plugins/chart.js/Chart.min.js"></script>

   <!-- PAGE SCRIPTS -->
   <script src="<?= base_url(); ?>assets/panel-master/dist/js/pages/dashboard2.js"></script>


   </body>

   </html>